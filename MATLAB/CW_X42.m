...
depth = CW_X42_data(:,2);
length = CW_X42_data(:,3);
lig_2 = CW_X42_data(:,4);
lig_1 = CW_X42_data(:,5);
burst_time = CW_X42_data(:,6);
...
% Burst criteria (MPa)
max_mises_1 = 395; % determined from simulation
...
if switch_linplt
    ...
    xlabel('${(d/t)^{0.366}(a/t)^{0.114}(l/t)^{-0.25}}$','interpreter','latex')
    ylabel('Normalized burst pressure')
    set(gca,'FontSize',40)
    set(gca,'YColor','k')
    set(gca,'LineWidth',2);
