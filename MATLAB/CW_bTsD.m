...
param = 'depth'; % Choose primary variables

% List out each individual parameters
depth = CW_bTsD_data(:,2);
length = CW_bTsD_data(:,3);
lig_2 = CW_bTsD_data(:,4);
lig_1 = CW_bTsD_data(:,5);
burst_time = CW_bTsD_data(:,6);
...
% Reference burst pressure for pipe without crack
pb_ref_1 = 140.07;

% Applied pressure in the simulations
p_sim = 140;
step1 = 0.8; % applied 80% pressure in the first step

% Convert time to pressure then normalize
pb_1 = (p_sim * step1 + p_sim * (1-step1) * burst_time) / pb_ref_1;
...
if switch_linplt
    ...
    xlabel('${(d/t)^{0.487}(a/t)^{0.184}(l/t)^{-0.445}}$','interpreter','latex')
    ylabel('Normalized burst pressure')
    set(gca,'FontSize',40)
    set(gca,'YColor','k')
    set(gca,'LineWidth',2);
