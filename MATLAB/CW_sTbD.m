...
param = 'length'; % Choose primary variables

% List out each individual parameters
depth = CW_sTbD_data(:,2);
length = CW_sTbD_data(:,3);
lig_2 = CW_sTbD_data(:,4);
lig_1 = CW_sTbD_data(:,5);
burst_time = CW_sTbD_data(:,6);

% Other parameters
t = 15; % pipe thickness (mm)
D = 440; % pipe outer diameter (mm)
...
p_sim = 42;
step1 = 0.75; % applied 75% pressure in the first step
...
if switch_linplt
    ...
    xlabel('${(d/t)^{0.51}(a/t)^{0.16}(l/t)^{-0.31}}$','interpreter','latex')
    ylabel('Normalized burst pressure')
    set(gca,'FontSize',40)
    set(gca,'YColor','k')
    set(gca,'LineWidth',2);
